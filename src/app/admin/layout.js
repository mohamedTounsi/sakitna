...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="min-h-screen flex bg-[#050b14] text-white" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* ── Desktop Sidebar ── */}
      <aside className="hidden md:flex w-64 bg-[#07101f] border-r border-white/8 flex-col flex-shrink-0">
        <SidebarContent />
      </aside>
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-72 bg-[#07101f] border-r border-white/8 flex flex-col z-50 md:hidden transform transition-transform duration-300 ${
      ...
      {/* ── Main content ── */}
      <main className="flex-1 overflow-y-auto min-h-screen bg-[#050b14] min-w-0">
        {/* Top bar */}
        <div className="border-b border-white/8 px-4 md:px-10 py-4 md:py-5 flex items-center justify-between gap-4">
          {/* Mobile: hamburger + live indicator */}
          <div className="flex items-center gap-3">
            <button
